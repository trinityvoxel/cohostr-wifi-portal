...
console.log(`[STARTUP] Webhook signature: ${DASHBOARD_WEBHOOK_SECRET ? 'enabled (HMAC-SHA256)' : 'disabled — set dashboard_webhook_secret to enable'}`);
...
async function notifyDashboard(name, email, mac, emailConsent) {
  try {
    ...
    // Build headers — sign with HMAC-SHA256 if a shared secret is configured
    const headers = {
      'Content-Type': 'application/json',
